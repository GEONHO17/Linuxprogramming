...
int main() {				//메인함수 
	...
	for (size_t i = 0; i < 5; i++) {	//반복문 
		string ch;						//문자열을 저장할 변수 
		cout << "문자열을 입력하세요: "; //메시지 출력 
		cin >> ch;					//입력받은 문자열 저장 
		v.push_back(ch);			//문자열 벡터 배열에 삽입 
	}								
	vector<string>::iterator it;	//iteratro 객체 생성 
	sort(v.begin(), v.end(),greater<string>());  //	내림차순으로 정렬 
	cout << "사전에서 제일 뒤에 나오는 단어는 "; //메시지 출력 
	for (it = v.begin(); it < v.begin() + 1; it++) //반복문 
		cout << *it << endl;		//메시지 출력 
